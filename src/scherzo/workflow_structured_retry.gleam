...
fn validator_retry_instruction(
  ...
  case list.any(validators, is_review_lane_draft_validator) {
    True ->
      ...
      <> "- Every `evidence_requests` item must include non-empty `request_id`, `draft_finding_id`, `evidence_key`, `claim`, and `expected_observation`, plus object `target`; `evidence_key` must use one of the keys documented by the review-lane provider schema. Use `context_only` with empty `target` for unsupported/manual review context; target may contain only `test_name`, `fixture_id`, `artifact_path`, `changed_file_path`, or `static_scan_rule` (do not include `command`, `suggested_test_file`, or `suggested_test_name`).\n"
    False -> ""
  }
}
