...
pub fn has_dispatch_blocker(
  ...
  |> task_lifecycle.has_dispatch_blocker(task_identity)
}
...
pub fn has_tracker_claim(
  ...
  projected_lifecycle(state)
  |> task_lifecycle.has_tracker_claim(task_identity)
}
                ...
                Ok(directory) ->
                  case
                    insert_retry_entries(
                      dict.to_list(state.runtime.retry_attempts),
                      directory,
                      refresh_generations,
                    )
                  {
...
fn projected_lifecycle(
  ...
  case from_transition_state(state) {
    Ok(directory) -> directory
    Error(_) -> state.lifecycle
  }
}
