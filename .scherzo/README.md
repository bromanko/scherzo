...
- Use `scripts/scherzo-pi` as the checked-in `pi.command` wrapper so workflows such as research and execplan can select `openai-codex/gpt-5.5:xhigh` while other workflows keep the default pi model.
...
# Optional. Defaults to openai-codex/gpt-5.5:xhigh for workflow:execplan.
export SCHERZO_EXECPLAN_PI_MODEL=openai-codex/gpt-5.5:xhigh
# Optional. PR defaults for workflow:execplan.
