...
# Optional. Overrides where workflow:execplan writes the primary Carbon HTML plan artifact. Defaults to $SCHERZO_RUN_ROOT/artifacts/execplan during a run, or tmp/execplan-artifacts for local helper use.
export SCHERZO_EXECPLAN_HTML_DIR=/path/to/execplan-html-artifacts
...
- `workflow:execplan` — uses repo-local exec-plan skills with `openai-codex/gpt-5.5:xhigh` to draft a Markdown source plan in `docs/plans/`, adversarially review it, incorporate the review, render the primary operator-facing Carbon HTML artifact, push a jj bookmark, open a ready GitHub PR, and create or reuse a follow-up `workflow:execplan-implementation` Linear issue in `Backlog`. The final validation and publish commands print `PRIMARY_PLAN_ARTIFACT=...` and `PLAN_HTML_PATH=...`; during daemon runs the default path is `$SCHERZO_RUN_ROOT/artifacts/execplan/<plan>.html`, which opens directly from disk and requires no local assets. The Markdown file remains the PR/source diff while the HTML file is the retained review artifact. The follow-up issue references the plan path and PR but stays out of Scherzo's active `Todo`/`In Progress` dispatch states until a human merges the plan PR and moves the implementation issue to `Todo`.
