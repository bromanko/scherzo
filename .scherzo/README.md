...
- Put versioned workflow definitions in `.scherzo/workflows/*.md`.
...
- Keep machine-specific variants as `.scherzo/workflows/*.local.md`; they are ignored by git.
- Do not put secrets in workflow files. Use environment variables for secrets and deployment-specific values.

The repo `.gitignore` intentionally ignores runtime `.scherzo/*` state while allowing this README and `.scherzo/workflows/**` to be checked in.
