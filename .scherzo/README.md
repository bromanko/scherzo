...
- `workflow:execplan-implementation` — validates a retained ExecPlan bundle from task context, implements from the prepared review document plus implementation pack, fails closed if they conflict, publishes a PR, and emits a retained `code_change_bundle` artifact. The broad implementation step rejects concrete blockers, no-op work, and invalid workspace evidence; an unblocked partial pass proceeds to independent semantic verification, whose bounded recovery continues the current canonical-plan backlog instead of immediately moving the task to Triage.
