...
- If synthesis output includes `REVIEW_SYNTHESIS_PATH=...` or `REVIEW_FINAL_ARTIFACT_PATH=...`, read the referenced `scripts/scherzo-review` artifacts first. Use the final artifact as the primary findings list, including lane failures, blocking findings, downgraded findings, and retained notes.
...
1. Read the post-plan-feedback change analysis output and staged review command outputs.
