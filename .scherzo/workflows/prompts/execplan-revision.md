...
Read `tmp/execplan-previous-bundle.json`, `tmp/execplan-previous-pack.json`, and the checked-in review document path in `tmp/execplan-review-doc.path`. Update the review document only when the feedback changes human-reviewable intent, scope, risk, rollout, milestones, or acceptance. If the feedback requires no change, leave the review document unchanged.
