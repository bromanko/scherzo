...
- [ ] Design Linear communication protocol for agent results, comments, and issue edits.
  ...
  Scherzo currently posts operational handoff comments such as claim, success, and failure, but it does not post the actual task result from pi. A future plan should define the higher-level protocol for what Scherzo writes back to Linear. Inbound operator control should remain local through `scherzoctl`, not Linear comments.

  The Scherzo-to-Linear result-comment half is now planned in `docs/plans/linear-session-results.md`. Historical Linear-to-Scherzo command/comment transport plans remain archived only; that transport has been removed.
  ...
  - Do not reintroduce Linear comments as an operator command transport; if future Linear UI affordances are designed, keep operator actions local through `scherzoctl` unless a separate design explicitly changes that boundary.
  - Define how edited issue descriptions, labels, and state changes interact with parked issues and running workers.
...
- [ ] Add Linear webhook wake-up for outbound/reporting workflows if needed.

  Inbound Linear comment commands are removed. Any future webhook work should be scoped to tracker freshness or outbound reporting needs, not as a replacement operator command transport.
