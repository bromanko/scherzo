...
`test/` remains excluded from the production Glinter profile, but `direnv exec . gleam run -m scherzo_lint` runs `scherzo_lint_test_determinism_guard` before the production pass. The guard blocks known flaky test patterns, including ad hoc `process.sleep`, tiny non-timeout `query_timeout_ms` literals, raw negative `process.receive` assertions/cases, hand-rolled receive drain loops, oversized table-driven EUnit tests, and global environment mutation in tests. For environment-dependent behavior, inject an env reader or pass an explicit child-process env; do not call `set_env`/`unset_env`, FFI `setenv`/`unsetenv`, or Erlang `os:putenv`/`os:unsetenv` from tests.
