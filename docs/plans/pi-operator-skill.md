...
- [ ] Add failing skill documentation tests in `test/skill_docs_test.gleam`.
- [ ] Add `.pi/skills/scherzo-operator/SKILL.md` and `.pi/skills/scherzo-operator/references/commands.md`.
- [ ] Update `README.md` with a `Using pi as an operator UI` section.
- [ ] Run format and test validation, recording the final pass count.
- [ ] Optionally run a manual pi skill-load dry-run and record the transcript shape or the reason it was skipped.

## Surprises & Discoveries

- Observation: The original plan's statement that the repository had no `.pi/skills/` directory is now stale.
  Evidence: The current tree has `.pi/skills/exec-plan/SKILL.md`, `.pi/skills/exec-plan-review/SKILL.md`, and several Gleam review skills. There is still no `.pi/skills/scherzo-operator/` directory.
...
- Observation: README already documents the control and observability CLI, including local mutating commands, but it does not document pi as an operator UI.
  Evidence: `README.md` has a `Control and observability` section with `scripts/scherzoctl` examples and no `scherzo-operator` or `/skill:scherzo-operator` text.
...
The plan has been revised against the current repository. Implementation of the `scherzo-operator` skill has not started in this working tree. At completion, record whether pi loaded the skill, whether `scripts/scherzoctl ps --json` and `events --json` were the first inspection commands in a manual dry-run, whether the README fallback for disabled skill commands was sufficient, and any gaps that suggest a future typed pi extension.
...
The current working tree has no `.pi/skills/scherzo-operator/` directory and no `test/skill_docs_test.gleam`. It does have `.pi/skills/` with other project skills, so this plan must add a new sibling directory rather than creating `.pi/skills/` from scratch.
...
`README.md` currently has `Control and observability`, `Linear command comments`, and safety sections. It does not mention `.pi/skills/scherzo-operator`, `/skill:scherzo-operator`, or `Using pi as an operator UI`.

At plan-revision time, `direnv exec . gleam test` passed with `533 passed, no failures`. Because this plan adds tests, the exact final count should be higher. If unrelated work changes the count, treat the pass/fail result and the presence of the new `skill_docs` tests as authoritative.
...
18. Commit the phase with a message such as `Add Scherzo operator pi skill`. One commit is sufficient because the change is additive documentation plus its validation tests; do not commit the intentionally failing red phase separately.
