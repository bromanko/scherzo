...
## Implemented WorkItemList query controls

`work_item_list` now accepts optional `state_filter`, `states`, `search`, `sort`, `limit`, and `cursor` request fields. `state_filter` supports `active`, `archive`, and `categories`. `active` expands to the non-terminal categories `backlog`, `ready`, `active`, and `unknown`; `archive` expands to the terminal categories `done`, `canceled`, and `duplicate`; `categories` requires a non-empty `states` array of category names. If `state_filter` is omitted and `states` is present, Scherzo treats the request as the explicit category filter for backward compatibility. If both are omitted, the default is `active`.

`search` is case-insensitive and matches the daemon WorkItem id, source display id when present, source provider id, title, and label names. Blank or whitespace-only search text is normalized away. `sort` currently supports `updated_desc`, which orders by `updated_at` descending, places missing timestamps after present timestamps, and breaks timestamp ties by daemon WorkItem id ascending so pagination stays deterministic.

WorkItem pagination cursors are now WorkItem-specific tokens that bind the offset to a normalized fingerprint of `state_filter`, `search`, and `sort`. Reusing a cursor with different query controls, or providing a malformed token, is rejected as `invalid_cursor` before adapter access. WorkItem reads remain provider-live with no daemon provider cache. The initial Linear adapter uses a bounded scoped scan plus post-filtering/search/sort for query shapes that cannot be reduced to configured Linear state names; pagination is terminal at that bounded scan window, so results beyond the current scan limit require a future provider-backed optimization. This keeps the contract tracker-neutral while documenting a scan-bound limitation for large projects. This feature does not change `workflows/dogfood/`, `.scherzo/workflows/schemas/`, or ExecPlan helper scripts.
