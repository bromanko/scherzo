...
- [ ] Execute the stabilization gate, record the generated `src` and `test` import/qualifier checklist in Progress, and update this plan if the tree has drifted.
- [ ] Move tracker issue types out of `src/scherzo/domain.gleam` and update tracker-facing consumers.
- [ ] Move configuration types out of `src/scherzo/domain.gleam` and update config/orchestrator consumers.
- [ ] Move result artifact, token total, live session, run attempt, and workspace record types to their owner modules.
- [ ] Move orchestrator runtime state types to `src/scherzo/orchestrator/state.gleam`.
- [ ] Remove the remaining `domain` dependencies, delete `src/scherzo/domain.gleam`, run full validation, and update the retrospective.
...
This section is intentionally empty for now because implementation has not begun. At the end of each milestone, record whether the milestone kept behavior unchanged, which greps and tests proved it, and whether any ownership decisions needed to change.
...
The current broad type module is `src/scherzo/domain.gleam`. It imports `birl` for `Time`, `gleam/dict` for dictionaries, `gleam/option` for optional fields, `src/scherzo/model_config.gleam` for model settings, `src/scherzo/orchestrator/reason.gleam` for park reasons, `src/scherzo/tracker/kind.gleam` for tracker kind, and `src/scherzo/tracker/state.gleam` for tracker issue states.
...
The current workflow-execution module is `src/scherzo/workflow_run.gleam`. A workflow run is one execution of a workflow DAG for one tracker issue. It currently imports `scherzo/domain` for issue, config, artifact limit, and token types. It also imports `src/scherzo/step_artifact.gleam`, `src/scherzo/workflow_scheduler.gleam`, `src/scherzo/workspace_run.gleam`, `src/scherzo/agent/run_attempt.gleam`, and `src/scherzo/agent/types.gleam`.
...
The current session modules are under `src/scherzo/session/`. `src/scherzo/session/event.gleam` defines session event names and statuses. `src/scherzo/session/json.gleam` encodes or decodes session data. `src/scherzo/session/hub.gleam` manages session event subscribers. `LiveSession` is currently not in that area; it lives in `src/scherzo/domain.gleam` and should move to `src/scherzo/session/live.gleam`.

The current agent modules are under `src/scherzo/agent/`. `src/scherzo/agent/types.gleam` defines worker success, worker failure, and pi update types. `src/scherzo/agent/run_attempt.gleam` contains the code that runs an agent prompt in a workspace. `RunAttempt` currently lives in `src/scherzo/domain.gleam` and should move into `src/scherzo/agent/run_attempt.gleam`.

The current orchestrator modules are under `src/scherzo/orchestrator/`. They coordinate polling, dispatch, worker registry, retry scheduling, effects, control commands, and daemon lifecycle. In-memory orchestrator state such as running issues, retry entries, issue counters, parked issues, invalid workflow reports, and token aggregation currently lives in `src/scherzo/domain.gleam`; those records should move to `src/scherzo/orchestrator/state.gleam`.

## Preconditions and Verified Facts

Before writing this plan, `jj status --color=never` reported a clean working copy with no changes. A filename-collision check for `docs/plans/LIV-24-*` found no existing plan file for this issue. The source tree currently contains `src/scherzo/domain.gleam`, `src/scherzo/workflow_dag.gleam`, `src/scherzo/workflow_run.gleam`, `src/scherzo/command_step.gleam`, `src/scherzo/step_artifact.gleam`, `src/scherzo/session/event.gleam`, `src/scherzo/agent/types.gleam`, `src/scherzo/state/record.gleam`, `src/scherzo/state/projection.gleam`, and `src/scherzo/state/recovery.gleam`.

The current `src/scherzo/domain.gleam` exports these type groups:
...
The current source import audit found `import scherzo/domain` in these source files: `src/scherzo/template.gleam`, `src/scherzo/session/json.gleam`, `src/scherzo/smoke.gleam`, `src/scherzo/config.gleam`, `src/scherzo/session/event.gleam`, `src/scherzo/step_artifact.gleam`, `src/scherzo/terminal/render.gleam`, `src/scherzo/orchestrator/workflow_reloader.gleam`, `src/scherzo/pi/protocol.gleam`, `src/scherzo/session/hub.gleam`, `src/scherzo/pi/client.gleam`, `src/scherzo/tracker.gleam`, `src/scherzo/workflow_run.gleam`, `src/scherzo/result_artifact.gleam`, `src/scherzo/linear_attachment.gleam`, `src/scherzo/control/linear_transport.gleam`, `src/scherzo/workspace_run.gleam`, `src/scherzo/state/projection.gleam`, `src/scherzo/linear_contract.gleam`, `src/scherzo/orchestrator/daemon.gleam`, `src/scherzo/orchestrator/core.gleam`, `src/scherzo/linear_triage.gleam`, `src/scherzo/control/protocol.gleam`, `src/scherzo/orchestrator/event_publisher.gleam`, `src/scherzo/command_step.gleam`, `src/scherzo/handoff_format.gleam`, `src/scherzo/runtime_bundle.gleam`, `src/scherzo/orchestrator/effect_runner.gleam`, `src/scherzo/orchestrator/worker_registry.gleam`, `src/scherzo/workspace.gleam`, `src/scherzo/workflow_policy.gleam`, `src/scherzo/agent/runner.gleam`, `src/scherzo/orchestrator/service.gleam`, `src/scherzo/linear.gleam`, `src/scherzo/handoff.gleam`, `src/scherzo/state/recovery.gleam`, `src/scherzo/agent/types.gleam`, `src/scherzo/agent/pi_rpc.gleam`, `src/scherzo/agent/run_attempt.gleam`, and `src/scherzo/agent/turn_loop.gleam`.
