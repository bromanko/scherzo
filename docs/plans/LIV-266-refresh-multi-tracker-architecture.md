...
- [x] (2026-05-14 03:20Z) LIV-269 added the adapter-contract half of Milestone 1: `test/tracker_adapter_test.gleam` and `src/scherzo/tracker/adapter.gleam`, including pure capability validation and exact missing-capability messages.
- [x] (2026-05-14 03:20Z) Milestone 1 adapter-contract validation passed with `direnv exec . gleam format --check src test`, `direnv exec . gleam test` reporting 1331 passed and no failures, `direnv exec . gleam run -m glinter` reporting 0 errors, and `direnv exec . gleam run -m scherzo_lint` reporting 0 errors.
