...
- `supporting_context` for the v2 drafting workflow is accepted only through formal Scherzo `mapped_output` inputs in the workflow contract. Linear issue URLs and attachments remain part of the issue brief for the agent to inspect, but v2 MVP helpers must not parse arbitrary artifact refs from issue text.
...
1. **Schemas.** Add JSON schemas for the durable v2 artifacts under `.scherzo/workflows/schemas/`:
   ...
   Add provider submission schemas under `.scherzo/workflows/schemas/` only for model-authored drafts that helper commands canonicalize:
   - `.scherzo/workflows/schemas/implementation-pack-submission.v2.schema.json`
   - `.scherzo/workflows/schemas/exec-plan-revision-submission.v2.schema.json`
...
- **Risk: schema placement makes workflow contracts depend on documentation paths.** Countermeasure: all new v2 canonical and provider schemas live under `.scherzo/workflows/schemas/`, the same workflow package that references them. Existing `docs/schemas/provider/` review-lane paths are treated as legacy/current precedent to avoid copying, not as guidance for v2.
...
- 2026-05-15: Use `.scherzo/workflows/schemas/` for both canonical workflow artifact schemas and provider submission schemas. The earlier `docs/schemas/provider/` guidance came from existing review-lane workflows and older plans; v2 does not extend that documentation path because the v2 workflows should be self-contained.
...
- Existing native review-lane provider schemas currently live under `docs/schemas/provider/`, while canonical workflow schemas also exist under `.scherzo/workflows/schemas/`. V2 follows the workflow-local `.scherzo/workflows/schemas/` location for all new schemas so the workflow package is self-contained.
...
- `.scherzo/workflows/schemas/` exists and already contains workflow-local schema files. `docs/schemas/provider/` also exists for current native review-lane provider schemas, but v2 must not add new schema files under `docs/`.
...
- Add canonical v2 artifact schemas and provider submission schemas under `.scherzo/workflows/schemas/`.
...
- Adding new v2 schema files under `docs/`; existing review-lane files there remain untouched.
- Parsing arbitrary retained artifact refs from Linear issue text for `supporting_context`; v2 MVP accepts supporting artifacts only through formal Scherzo mapped outputs.
...
First, add schemas and fixtures under `.scherzo/workflows/schemas/` and `test/fixtures/execplan_v2/`. A canonical artifact schema must be strict about `schema_version`, `artifact_type`, required hashes, repository-relative paths, and artifact refs. Provider schemas may be less rich than canonical schemas, because helper commands will add provenance, hashes, refs, and handoff metadata. Do not create or extend a v2 schema location under `docs/`.
...
2. Create schema files and fixtures.
   - Add `.scherzo/workflows/schemas/exec-plan-bundle.v2.schema.json`.
   - Add `.scherzo/workflows/schemas/implementation-pack.v2.schema.json`.
   - Add `.scherzo/workflows/schemas/code-change-bundle.v2.schema.json`.
   - Add `.scherzo/workflows/schemas/implementation-pack-submission.v2.schema.json`.
   - Add `.scherzo/workflows/schemas/exec-plan-revision-submission.v2.schema.json`.
...
3. Define canonical `implementation_pack` fields.
   - In `.scherzo/workflows/schemas/implementation-pack.v2.schema.json`, require:
     - `schema_version: 2`
...
4. Define canonical `exec_plan_bundle` fields.
   - In `.scherzo/workflows/schemas/exec-plan-bundle.v2.schema.json`, require:
     - `schema_version: 2`
...
5. Define canonical `code_change_bundle` fields.
   - In `.scherzo/workflows/schemas/code-change-bundle.v2.schema.json`, require:
     - `schema_version: 2`
...
11. Implement review-doc publication, bundle materialization, and handoff.
    ...
    - It creates or reuses a Linear implementation issue labeled `workflow:execplan-implementation-v2` in the same default `Backlog` state used by v1 handoff issues, unless the repository's existing Linear defaulting logic supplies that state automatically.
...
14. Add `.scherzo/workflows/execplan-v2.yaml`.
    ...
    - Include this contract shape. The only deterministic `supporting_context` source in the v2 MVP is `mapped_output`; do not add helper parsing of artifact refs from issue body text, Linear URLs, or attachments.
      ```yaml
      contract:
        version: 1
        inputs:
...
15. Add v2 revision support.
    ...
    - A human creates or updates a Linear issue with `workflow:execplan-revision-v2`, `Bundle ref:`, `Bundle sha256:`, and the actionable PR feedback to apply. The workflow consumes that issue context, prepares the PR branch, validates the previous bundle, lets the agent revise the review doc and pack submission, materializes a superseding bundle, publishes the branch when changed, and acknowledges feedback.
    ...
    - The revision agent must use structured output `implementation_pack_submission` with provider schema `.scherzo/workflows/schemas/exec-plan-revision-submission.v2.schema.json`.
...
18. Add workflow parse and fingerprint coverage.
    ...
    - Add a workflow configuration assertion that the three v2 workflow files reference schema paths under `.scherzo/workflows/schemas/` and do not reference `docs/schemas/`.
...
19. Add implementation output generation tests.
    ...
    - Assert the output validates against `.scherzo/workflows/schemas/code-change-bundle.v2.schema.json` and includes PR URL, branch, base/head revisions, changed files, diff ref/hash, validation artifact refs, plan-completion verdict, and review artifact refs.
...
- **Schema validation:** valid fixture artifacts validate against their schemas; fixtures with missing required fields, non-relative paths, or invalid artifact refs fail. All new v2 schema paths are under `.scherzo/workflows/schemas/`; no v2 workflow references `docs/schemas/`.
...
- A v2 helper accepts `supporting_context` by scraping arbitrary artifact refs from Linear issue text instead of through a formal `mapped_output` contract input.
...
5. Confirm the generated implementation handoff issue is in `Backlog`, then run it with `workflow:execplan-implementation-v2` only after the plan PR is merged and a human moves it to `Todo`.
6. Try one revision pass by having a human create or update a Linear issue labeled `workflow:execplan-revision-v2` with the bundle ref/hash and actionable feedback.
...
- `exec_plan_bundle`: `.scherzo/workflows/schemas/exec-plan-bundle.v2.schema.json`
- `implementation_pack`: `.scherzo/workflows/schemas/implementation-pack.v2.schema.json`
- `code_change_bundle`: `.scherzo/workflows/schemas/code-change-bundle.v2.schema.json`
...
- `.scherzo/workflows/schemas/*.schema.json`: v2 canonical artifact and provider submission schemas. New v2 schemas must stay workflow-local and self-contained, not under `docs/`.
