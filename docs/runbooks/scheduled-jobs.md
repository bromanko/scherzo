...
4. Use a workspace driver that can publish changes (`publish-change`) back to the same repository branch. The resolver assumes it may fast-forward the PR head branch in the configured repository; fork PRs are rejected.
