...
4. Use a workspace driver that can publish commit stacks (`publish-commit-stack`) back to the same repository branch. The resolver assumes Scherzo core may fast-forward the PR head branch in the configured repository; fork PRs are rejected.
