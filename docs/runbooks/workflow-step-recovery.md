...
This page documents the active repair-and-recheck step-recovery path in Scherzo. When a step has effective `recover` configuration and its failure would otherwise be fatal, `workflow_run` records the failed original attempt, writes a structured recovery-input artifact, starts one bounded nested recovery worker in the same workspace, and rechecks the original step unchanged when the recovery result is `recheck`.
