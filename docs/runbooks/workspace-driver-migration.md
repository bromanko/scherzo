...
Command steps and agent subprocesses run under a selected driver receive `SCHERZO_WORKSPACE_PROFILE=isolated`, `SCHERZO_WORKSPACE_DRIVER=<resolved-driver-command>`, and `SCHERZO_WORKSPACE_CAPABILITIES="status diff changed-files assert-only baseline refresh-base publish-change"` for the current bundled jj driver. A custom same-repo commit-stack driver may report `publish-commit-stack` instead. An original agent prompt can render `{{ workspace.driver }}` or loop over `{% for capability in workspace.capabilities %}`.
