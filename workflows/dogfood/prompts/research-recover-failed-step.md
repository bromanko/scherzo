...
Return `gave_up` instead of `recheck` when the failure is caused by missing or broken workflow configuration, an unsafe unexpected source change, insufficient evidence for an accurate report, possible remote side effects, or any other condition you cannot safely fix locally.
