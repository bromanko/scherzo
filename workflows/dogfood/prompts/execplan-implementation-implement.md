...
Do not stop early or knowingly submit incomplete work merely because bounded step recovery exists: this `implement_plan` step remains the primary broad implementation pass. If the verifier nevertheless identifies explicit unfinished canonical-plan work, recovery is authorized to treat those findings as an actionable backlog and complete broad or multi-file work within its configured attempt budget. If known required Progress items, blocking acceptance criteria, or implementation-pack requirements remain now, keep implementing instead of stopping, unless you are blocked by an explicit conflict, missing required handoff artifact, unsafe ambiguity, or required product decision. Treat any repo path in `review_surface_path` or `plan_publication_path` as optional human-review metadata, not as the authoritative implementation input.
...
- When the plan requires an unchanged-surface, helper, touched-surface, or implementation inventory, populate `implementation_inventory` explicitly. Give each surface a repository glob, disposition, and evidence; do not replace this inventory with a generic statement that nothing else changed.
- If any criterion lacks current-tree evidence, any required focused path is untested, a legacy field remains, a new owner is declared but unused, a shrinking guardrail misses its baseline, or a required inventory is absent, set `ready_for_verification: false` and name the gap in `remaining_required_work`.
