...
- Treat a plan-required `jj diff --check` as a diff-integrity requirement, not a requirement for a CLI flag that may not exist in the installed jj version. If jj rejects that flag as unsupported, run `git diff --check` and `jj diff --git --from <base_change_id> --to @ --color=never | git apply --check --allow-empty -` instead. When both pass, record the compatibility substitution and treat the diff-integrity criterion as satisfied; unsupported `jj diff --check` is not an infrastructure blocker.
