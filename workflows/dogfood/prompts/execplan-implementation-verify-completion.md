...
- Inspect the smallest useful set of changed files and tests needed to verify promised behavior. When you inspect the workflow-wide diff, use the immutable `base_commit_id` in `$SCHERZO_RUN_ROOT/state/implementation/metadata.json`; `base_source_revision` is mutable provenance only. Use current workspace evidence rather than historical prompt transcripts.
...
2. Check the current workspace and changed files/tests from the metadata-pinned `base_commit_id` against the canonical plan and implementation pack. Reconcile any partial diagnostic backlog against current-tree evidence.
