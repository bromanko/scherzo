...
- The final checkpoint/gate commands fail closed if the structured submission is missing or malformed, if the stamped verdict has verdict `fail`, or if the workspace changes before the gate stamps current context. If that final gate still reports a fresh `fail`, Scherzo will retain the workspace and write diagnostic recovery artifacts rather than launching another automatic edit pass.
