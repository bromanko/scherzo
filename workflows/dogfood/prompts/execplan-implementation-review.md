...
- Use the native staged review artifacts produced by Scherzo `kind: agent` lane steps plus `: "${SCHERZO_WORKFLOW_BUNDLE_DIR:?Scherzo command-step contract missing SCHERZO_WORKFLOW_BUNDLE_DIR}"; "$SCHERZO_WORKFLOW_BUNDLE_DIR/scripts/scherzo-review"` normalization command steps as the specialist review input. If `REVIEW_FINAL_ARTIFACT_PATH=...` is present, treat that final artifact as the primary findings list, including lane failures, blocking findings, downgraded findings, and retained notes.
