...
case "$input" in
  *'"operation":"task_source.fetch_candidates"'*)
    case "$scenario" in
      fetch-wrong-backend)
        printf '{"schema_version":1,"request_id":"%s","ok":true,"result":{"tasks":[%s]}}\n' "$(printf '%s' "$input" | sed -n 's/.*"request_id":"\([^"]*\)".*/\1/p')" "$wrong_backend_task"
        ;;
      *)
        printf '{"schema_version":1,"request_id":"%s","ok":true,"result":{"tasks":[%s]}}\n' "$(printf '%s' "$input" | sed -n 's/.*"request_id":"\([^"]*\)".*/\1/p')" "$task"
        ;;
    esac
    ;;
  ...
  *'"operation":"task_source.refresh_by_refs"'*)
    case "$scenario" in
      unstable-identity)
        ...
        printf '{"schema_version":1,"request_id":"%s","ok":true,"result":{"tasks":[%s]}}\n' "$(printf '%s' "$input" | sed -n 's/.*"request_id":"\([^"]*\)".*/\1/p')" "$bad_task"
        ;;
      *)
        printf '{"schema_version":1,"request_id":"%s","ok":true,"result":{"tasks":[%s]}}\n' "$(printf '%s' "$input" | sed -n 's/.*"request_id":"\([^"]*\)".*/\1/p')" "$task"
        ;;
  ...
  *'"operation":"task_source.lookup_by_operator_ref"'*)
    case "$scenario" in
      lookup-known-wrong)
        printf '{"schema_version":1,"request_id":"%s","ok":true,"result":{"task":%s}}\n' "$(printf '%s' "$input" | sed -n 's/.*"request_id":"\([^"]*\)".*/\1/p')" "$wrong_identity_task"
        ;;
      *)
        printf '{"schema_version":1,"request_id":"%s","ok":true,"result":{"task":%s}}\n' "$(printf '%s' "$input" | sed -n 's/.*"request_id":"\([^"]*\)".*/\1/p')" "$task"
        ;;
