...
`direnv exec . gleam test` and `direnv exec . scherzo-test-unit` run the deterministic unit suite. Shell-heavy agent-runner/worker-control, helper-script, workflow, workstream-start, structured-output validator, renderer, daemon/service, remote-registration, workspace/hook, port/process, pi-client, CLI, tracker-conformance, adapter-doc conformance, and workspace-driver contract tests are excluded from that default loop and run with `direnv exec . scherzo-test-contract` (or `gleam test -- --suite contract`).
