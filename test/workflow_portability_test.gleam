...
fn execplan_v2_prompt_paths() -> List(String) {
  [
...
fn retired_v1_paths() -> List(String) {
  [
...
pub fn execplan_v1_workflow_files_are_retired_test() {
  list.each(retired_v1_paths(), fn(path) {
    let assert Ok(False) = simplifile.is_file(path)
  })
}

pub fn execplan_v2_workflows_resolve_helpers_from_repo_root_test() {
  list.each(
    [
    ...
    fn(path) {
      ...
      assert_contains(workflow, "\"$repo_root/scripts/scherzo-execplan-v2\"")
      assert_not_contains(workflow, "run: scripts/scherzo-execplan")
    },
  )
}

pub fn execplan_v2_family_prompts_do_not_reference_repo_local_skill_files_test() {
  list.each(execplan_v2_prompt_paths(), fn(path) {
...
pub fn review_workflows_use_staged_artifacts_instead_of_local_review_skills_test() {
  ...
  let execplan_implementation_workflow =
    read_file(".scherzo/workflows/execplan-implementation-v2.yaml")
  let implementation_prompt =
    read_file(".scherzo/workflows/prompts/code-review.md")
  let execplan_prompt =
    read_file(".scherzo/workflows/prompts/execplan-implementation-v2-review.md")
...
pub fn execplan_v2_prompts_describe_bundle_handoff_test() {
  let draft_prompt =
    read_file(".scherzo/workflows/prompts/execplan-v2-draft.md")
  let review_prompt =
    read_file(".scherzo/workflows/prompts/execplan-v2-review.md")
  let incorporate_prompt =
    read_file(".scherzo/workflows/prompts/execplan-v2-incorporate-review.md")
  let implementation_prompt =
    read_file(
      ".scherzo/workflows/prompts/execplan-implementation-v2-implement.md",
    )
  let revision_prompt =
    read_file(".scherzo/workflows/prompts/execplan-revision-v2.md")

  assert_contains(draft_prompt, "workflow:execplan-v2")
  assert_contains(draft_prompt, "submit_implementation_pack_submission")
  assert_contains(draft_prompt, "Bundle ref:")
  assert_contains(review_prompt, "workflow:execplan-v2")
  assert_contains(incorporate_prompt, "implementation pack")
  assert_contains(implementation_prompt, "workflow:execplan-implementation-v2")
  assert_contains(implementation_prompt, "tmp/execplan-v2-review-doc.md")
  assert_contains(
    implementation_prompt,
    "tmp/execplan-v2-implementation-pack.json",
  )
  assert_contains(implementation_prompt, "tmp/execplan-v2-conflict.md")
  assert_contains(revision_prompt, "workflow:execplan-revision-v2")
  assert_contains(revision_prompt, "Bundle ref:")
}

pub fn workflow_docs_explain_v2_only_routing_and_validation_test() {
  let docs = read_file(".scherzo/README.md")

  assert_contains(docs, "## Workflow-packaged guidance and portability")
  assert_contains(docs, "ExecPlan v2 workflows")
  assert_contains(docs, ".scherzo/workflows/schemas/")
  assert_contains(docs, "repo_root=${SCHERZO_REPO_ROOT:-$(cd")
  assert_contains(docs, "\"$repo_root/scripts/scherzo-execplan-v2\"")
  ...
  assert_contains(docs, "workflow:execplan-v2")
  assert_contains(docs, "workflow:execplan-revision-v2")
  assert_contains(docs, "workflow:execplan-implementation-v2")
  assert_contains(docs, "legacy v1 labels `workflow:execplan`")
  assert_not_contains(docs, "`workflow:execplan` —")
  assert_not_contains(docs, "workflow:execplan-implementation` —")
